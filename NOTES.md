...
- Stranded orphan files (unused after the redesign deleted their consumers; tree-shaken out, no broken imports): `store/recentlyViewed.ts`, `store/wishlist.ts`, `content/features.ts`, `content/specs.ts`, `components/ui/ScrollReveal.tsx`. Safe to delete in a future cleanup pass. (`components/ui/Card.tsx` was already an orphan pre-redesign.)
